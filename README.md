...
- **未信任的人（新用户，或你还没回复过的人）发的每一条消息先过判定**（先本地强规则：含 `t.me/xxxbot`、`?start=` 等明显广告特征直接判广告、不调 AI；否则交给 AI）：
  - 判为广告 → 进「🚫 广告拦截」隔离话题；若 `AUTO_BLOCK="1"`（默认）则**自动拉黑该用户**：他后续消息一律静默丢弃，直到你 `/allow` 放行（多次发广告的号只会留一条，不再骚扰）。设 `AUTO_BLOCK="0"` 则只拦不拉黑。
...
- **只认最高管理员**：群里只有 `ADMIN_USER_ID` 本人发的消息/命令才生效；万一别人被拉进管理群也无法操作或冒充你回复。
- **webhook 校验 fail-closed**：未带正确 `WEBHOOK_SECRET` 的请求一律拒绝（连"没配置就跳过"的情况都拒），杜绝伪造消息。
- **本地强规则预筛**：明显广告不调 AI，省成本，也防 AI 提示注入绕过。
- **写节流**：熟人消息的 `lastSeen` 6 小时内不重复写 KV，避免触及免费额度。
- **自动清理**：过期的未回复话题 + 90 天没动静的拉黑记录都会清，防 KV 堆积。
