...
- **图标按钮验证**：陌生人首次私聊先做一道「点出指定图标」的按钮题（默认两排 8 个图标，可调 8–20）；点错有第二次机会、换一道不同的题，两次都错才拦截。纯发广告的脚本不点按钮，直接挡在门外。
- **图形化配置菜单**：管理员私聊 bot 发 `/start`，在线开关/调整验证、关键词屏蔽、自动回复、内容类型过滤，全部内联按钮操作。
...
| 子菜单 | 能做什么 |
|---|---|
| 📝 基础配置（验证） | 开关人机验证、编辑欢迎语、调整验证图标个数（8–20，每排 4 个） |
...
> 验证是「点出指定图标」的按钮题，真人一点即过、零摩擦。两次都点错会被拦截，并往「🚫 广告拦截」话题发一条带 `/allow <ID>` 的通知，方便真人误触时你放行。验证开关默认开启，可在「基础配置」关闭。
...
- 别人正常私聊 → 先收到图标按钮验证题，点对后才会在群里建话题、你回复即可对话。
...
1. **验证门**（可在菜单关闭）：新人先收欢迎语 + 一道图标按钮题并置 `pending_verification`（不建话题、不调 AI）。点对置 `verified`；点错给第二次机会换一道不同的题，两次都错则 `is_blocked` 拦截并往「🚫 广告拦截」话题通知（带 `/allow`）。正确项以随机 token 存 D1，按钮上看不出对错，打字不算作答只重发新题。
